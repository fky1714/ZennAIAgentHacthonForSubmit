...
project_root_candidates = [
    ...
    ),  # task_solution directory
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..")
    ),  # One level above task_solution (if scripts is a subdir)
]
project_root = None
...
if (
    os.path.join(project_root, "task_solution") not in sys.path
):  # Ensure task_solution itself is accessible for relative imports if needed
...
EMBEDDING_MODEL_NAME = (
    "text-embedding-005"  # Changed to latest stable text embedding model
)
CHUNK_SIZE = 1000
CHUNK_OVERLAP = 100
...
try:
    ...
    logger.info(
        f"Vertex AI initialized for project '{gcp_project_id}' and location '{gcp_location}'."
    )
    ...
    embedding_model = TextEmbeddingModel.from_pretrained(
        EMBEDDING_MODEL_NAME
    )  # Uses the updated model name
    logger.info(
        f"Successfully initialized Firestore service and Embedding model '{EMBEDDING_MODEL_NAME}'."
    )
...
def get_all_reports(uid: str) -> List[Dict[str, Any]]:
    ...
    while True:
        try:
            ...
            paged_reports = firestore_service.get_reports(
                uid, page=page, page_size=page_size
            )
            ...
            logger.error(
                f"Error fetching reports for UID {uid} on page {page}: {e}",
                exc_info=True,
            )
            break
...
def get_all_procedures(uid: str) -> List[Dict[str, Any]]:
    ...
    while True:
        try:
            ...
            paged_procedures = firestore_service.get_procedures(
                uid, page=page, page_size=page_size
            )
            ...
            logger.error(
                f"Error fetching procedures for UID {uid} on page {page}: {e}",
                exc_info=True,
            )
            break
...
def delete_existing_chunks(uid: str):
    logger.info(
        f"Deleting existing RAG chunks for UID: {uid} from '{TARGET_RAG_COLLECTION_NAME}'..."
    )
    ...
    for i, doc in enumerate(docs_snapshot):
        ...
        if deleted_count % FIRESTORE_BATCH_SIZE == 0:
            batch.commit()
            logger.info(
                f"Committed batch delete, {deleted_count}/{total_to_delete} docs deleted so far."
            )
            batch = db.batch()
...
def prepare_and_embed_chunks(
    text_content: str, splitter: MarkdownTextSplitter, metadata: Dict[str, Any]
) -> List[Dict[str, Any]]:
    chunks_with_embeddings = []
    source_text_identifier = metadata.get(
        "title", metadata.get("original_doc_id", "Unknown source")
    )
    ...
    if not text_chunks:
        logger.warning(
            f"No text chunks generated for '{source_text_identifier}'. Skipping."
        )
        return []

    logger.info(
        f"Embedding {len(text_chunks)} chunks for '{source_text_identifier}' one by one..."
    )
    ...
    for i, chunk_text in enumerate(text_chunks):
        try:
            ...
            embedding_response = embedding_model.get_embeddings(
                [embedding_input]
            )  # Pass list of TextEmbeddingInput

            if (
                embedding_response
                and hasattr(embedding_response[0], "values")
                and embedding_response[0].values
            ):
                ...
                first_few_vals = (
                    str(emb_val[:3])
                    if emb_len > 0 and isinstance(emb_val, list)
                    else "N/A"
                )
                ...
                if (
                    emb_len != 768
                ):  # Check for text-embedding-005 model's expected dimension (768)
                    logger.error(
                        f"  CRITICAL: Embedding for chunk {i+1} of '{source_text_identifier}' has unexpected dimension {emb_len}. Expected 768 for {EMBEDDING_MODEL_NAME}. Values (first 3): {first_few_vals}"
                    )
                ...
                logger.error(
                    f"  Failed to get embedding (or 'values' attribute missing/empty) for chunk {i+1} of '{source_text_identifier}'. Response object: {str(embedding_response)[:200]}"
                )
        except Exception as e:
            logger.error(
                f"  Exception while embedding chunk {i+1} of '{source_text_identifier}': {e}",
                exc_info=True,
            )
            # エラーが発生したチャンクはスキップ
    ...
    if successful_embeddings_count < len(text_chunks):
        logger.warning(
            f"Successfully embedded {successful_embeddings_count} out of {len(text_chunks)} chunks for '{source_text_identifier}'."
        )

    if not chunks_with_embeddings:
        logger.error(
            f"No chunks were successfully embedded for '{source_text_identifier}'."
        )
...
def batch_write_to_firestore(chunks_to_write: List[Dict[str, Any]]):
    ...
    logger.info(
        f"Starting batch write of {len(chunks_to_write)} chunks to '{TARGET_RAG_COLLECTION_NAME}'..."
    )
    target_collection_ref = db.collection(TARGET_RAG_COLLECTION_NAME)
    batch = db.batch()
    written_count = 0
    for i, chunk_doc in enumerate(chunks_to_write):
        ...
        if (i + 1) % FIRESTORE_BATCH_SIZE == 0:  # Commit batch
            batch.commit()
            logger.info(
                f"Committed batch write, {written_count}/{len(chunks_to_write)} chunks written."
            )
            batch = db.batch()
    ...
    if written_count % FIRESTORE_BATCH_SIZE != 0:  # Commit any remaining writes
        batch.commit()
    logger.info(
        f"Finished batch write. Total {written_count} chunks written to '{TARGET_RAG_COLLECTION_NAME}'."
    )
...
def process_user_data(uid: str):
    ...
    if not reports:
        ...
        for report in reports:
            report_id = report.get(
                "id", f"unknown_report_id_{len(all_chunk_docs_to_write)}"
            )
            title = report.get("title", "")
            content = report.get("content", "")
            logger.info(
                f"  Processing Report ID: {report_id}, Title: '{title[:50]}...'"
            )

            markdown_content = f"# {title}\n\n{content}" if title else content
            if not content.strip() and not title.strip():
                logger.warning(
                    f"  Report ID: {report_id} has no title or content. Skipping."
                )
                continue

            metadata = {
                "original_uid": uid,
                "original_doc_type": "report",
                "original_doc_id": report_id,
                "title": title,
                "created_at": report.get("created_at"),
                "updated_at": report.get("updated_at"),
            }
            report_chunks = prepare_and_embed_chunks(
                markdown_content, text_splitter, metadata
            )
            all_chunk_docs_to_write.extend(report_chunks)
            logger.debug(
                f"    Generated {len(report_chunks)} chunks for Report ID: {report_id}"
            )
    ...
    if not procedures:
        ...
        for procedure in procedures:
            task_name = procedure.get(
                "task_name", f"unknown_proc_id_{len(all_chunk_docs_to_write)}"
            )
            ...
            if not content.strip():
                logger.warning(
                    f"  Procedure Task Name: {task_name} has no content. Skipping."
                )
                continue

            metadata = {
                "original_uid": uid,
                "original_doc_type": "procedure",
                "original_doc_id": task_name,
                "title": task_name,
                "created_at": procedure.get("created_at"),
                "updated_at": procedure.get("updated_at"),
            }
            procedure_chunks = prepare_and_embed_chunks(
                markdown_content, text_splitter, metadata
            )
            all_chunk_docs_to_write.extend(procedure_chunks)
            logger.debug(
                f"    Generated {len(procedure_chunks)} chunks for Procedure: {task_name}"
            )
    ...
    if all_chunk_docs_to_write:
        ...
        logger.info(
            f"No new chunks generated for UID: {uid}. Nothing to write to Firestore."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Prepare RAG data from Firestore Reports and Procedures for a specific user."
    )
    ...
    try:
        ...
        logger.error(
            f"An error occurred during script execution for UID {args.uid}: {e}",
            exc_info=True,
        )
        sys.exit(1)
