...
def retrieve_work_reports(
    user_query: str, uid: str, offset: int = 0, limit: int = 10
) -> list[str]:
    """
    ...
    """
    print(
        f"DEBUG: Retrieving work reports from Firestore for uid='{uid}', query='{user_query}', offset={offset}, limit={limit}"
    )
    ...
    try:
        ...
        page = (offset // limit) + 1
        reports_data = firestore_service.get_reports(
            uid=uid, page=page, page_size=limit
        )
...
def retrieve_manuals(
    ...
    """
    print(
        f"DEBUG: Retrieving manuals from Firestore for uid='{uid}', query='{user_query}', offset={offset}, limit={limit}"
    )
    if not firestore_service:
    ...
    try:
        ...
        page = (offset // limit) + 1
        manuals_data = firestore_service.get_procedures(
            uid=uid, page=page, page_size=limit
        )
...
if __name__ == "__main__":
    ...
    if not firestore_service:
        ...
        print(
            " Please ensure Firestore emulator is running and GOOGLE_APPLICATION_CREDENTIALS is set,"
        )
        print(
            " or that the application is running in an environment with Firestore access."
        )
    else:
        print(
            "Firestore_service is available. Running live tests (requires Firestore emulator or live connection)."
        )
        ...
        if reports:
            ...
            print(
                "No work reports retrieved. Ensure data exists in Firestore for the test UID or check Firestore connection."
            )
        ...
        if manuals:
            ...
            print(
                "No manuals retrieved. Ensure data exists in Firestore for the test UID or check Firestore connection."
            )

        # オフセットのテスト
        print("\n--- Retrieving Work Reports with Offset ---")
        reports_offset = retrieve_work_reports(
            "offset query", uid=test_uid, offset=5, limit=5
        )
        if reports_offset:
        ...
        reports_non_existent = retrieve_work_reports(
            "query", uid=non_existent_uid, limit=2
        )
