...
echo "  --field-config=field-path=metadata.original_uid,order=ASCENDING \\"
echo "  --field-config=field-path=metadata.original_doc_type,order=ASCENDING \\"
echo "  --field-config=field-path=${EMBEDDING_FIELD},vector-config='{\"dimension\":\"${EMBEDDING_DIMENSION}\",\"flat\":\"{}\"}' ${GCLOUD_PROJECT_OPTION}"
echo "---------------------------------------------------------------------"
...
if [[ "${confirmation}" =~ ^[Yy]$ ]]; then
  ...
  gcloud alpha firestore indexes composite create \
    ...
    --field-config=field-path=metadata.original_uid,order=ASCENDING \
    --field-config=field-path=metadata.original_doc_type,order=ASCENDING \
    --field-config=field-path=${EMBEDDING_FIELD},vector-config='{"dimension":"'${EMBEDDING_DIMENSION}'","flat":"{}"}' ${GCLOUD_PROJECT_OPTION}
